...
export class SessionManager {
    ...
    static async invalidateAllUserSessions(userId, excludeSessionId) {
        try {
    ...
    // Add session to user's session list
    static async addSessionToUserList(userId, sessionId) {
        try {
            const userSessionsKey = `${this.USER_SESSIONS_PREFIX}${userId}`;
            const sessionIds = await cacheUtils.get(userSessionsKey) || [];
    ...
    static async removeSessionFromUserList(userId, sessionId) {
        try {
            const userSessionsKey = `${this.USER_SESSIONS_PREFIX}${userId}`;
            const sessionIds = await cacheUtils.get(userSessionsKey) || [];
...
SessionManager.TOKEN_BLACKLIST_PREFIX = 'blacklisted_token:';
SessionManager.USER_SESSIONS_PREFIX = 'user_sessions:';
SessionManager.SESSION_DATA_PREFIX = 'session_data:';
SessionManager.TOKEN_BLACKLIST_TTL = 7 * 24 * 60 * 60; // 7 days (max refresh token lifetime)
SessionManager.SESSION_TTL = 24 * 60 * 60; // 24 hours
