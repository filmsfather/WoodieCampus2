...
import { corsOptions, helmetOptions, generalRateLimit, requestLogger, securityHeaders, securityErrorHandler } from './middleware/security.js';
...
// Health check endpoint with database, Redis, and Socket.io status
app.get('/health', async (req, res) => {
    const dbConnected = await checkDatabaseConnection();
    const redisConnected = await checkRedisConnection();
    const connectedUsers = await socketUtils.getConnectedUsers();
    const status = dbConnected && redisConnected ? 'OK' : 'PARTIAL';
