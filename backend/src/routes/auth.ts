...
router.get('/me', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT id, email, first_name, last_name, role, is_active, is_verified, created_at, updated_at FROM users WHERE id = $1',
      [req.user.userId]
    );
...
router.post('/logout-all-others', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
...
router.post('/token-status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
